...
Each target is `name -> (output path, render() -> str, committed?)`. Committed
targets are checked into git and verified by --check; each lives where its
consumer reads it — publications.yml in _data/ for Jekyll, papers.bib in cv/ for
latexmk — so there is no copy step. The rest are build outputs (gitignored) that
are only ever written. Adding a section = one line here.
"""
import argparse
import os
import sys
...
TARGETS = {
    ...
    'publications-bib':  ('cv/papers.bib',                         _pub_bib,                 True),
